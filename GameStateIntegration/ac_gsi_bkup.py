...
class AC_GSI:
    ...
    # using point cloud data, create minimap. fill strategy? unsure.
        ...
        # align centers
        view_center = (250,250)
        map_center = ((x_min+x_max)/2, (y_min+y_max)/2)
        # map_center = ((x_min+x_max)/2*sr, (y_min+y_max)/2*sr)
        offset = (view_center[0]- map_center[0],view_center[1]- map_center[1]) # offset 
        # offset = ( map_center[0]-view_center[0], map_center[1]-view_center[1]) # offset 
        for i in range(length):
            A[0, :] = A[0, :] + offset[0] # x
            A[1, :] = A[1, :] + offset[1] # y
